...
Cel::Renderer::Camera::update_view_matrix(const glm::mat4& transform)
{
    ...
    // Remove the scaling from the transform, so we juts have rotation &
    // translation
    ...
    // Reconstruct a mat4 from just the rotation & translation
...
Cel::Renderer::Camera::get_projection_matrix(const VkExtent2D extent) const
{
    ...
    return glm::perspective(glm::radians(fov), aspect, farPlane, nearPlane);
}
